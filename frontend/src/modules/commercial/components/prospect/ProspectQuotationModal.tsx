...
export default function ProspectQuotationModal({
  ...
  const serviceCatalog = useMemo(
    () => collectionResults<ServiceCatalog>(serviceCatalogQuery.data),
  ...
  function handleServiceChange(key: string, serviceCatalogId: string) {
    ...
    setItems((current) => current.map((item) => (
      item.key === key
        ? { ...item, service_catalog_id: serviceCatalogId, description: service?.service_name ?? '' }
        : item
    )))
  }
      ...
      >
        <header className="prospect-modal-header">
          <div>
            ...
            <h3 id="prospect-quotation-modal-title">Nueva cotización</h3>
          </div>
        ...
        <div className="prospect-modal-body">
          <p className="prospect-modal-notice">
            Esta cotización puede emitirse antes de convertir el prospecto en cliente. En ese caso, cliente permanecerá como nulo.
          </p>
          ...
          {!isLoading && opportunities.length === 0 && (
            <div className="prospects-error" role="alert">
              Este prospecto todavía no tiene una oportunidad. Crea primero la oportunidad comercial.
            </div>
          )}
          ...
          {!isLoading && opportunities.length > 0 && (
            <form onSubmit={handleSubmit}>
              <div>
                <label htmlFor="prospect-quotation-number">Número de cotización</label>
                <input
                  id="prospect-quotation-number"
                  value={quotationNumber}
                  onChange={(event) => setQuotationNumber(event.target.value)}
                  placeholder="COT-2026-000007"
                  required
                />
              </div>

              <div>
                <label htmlFor="prospect-quotation-opportunity">Oportunidad</label>
                <select
                  id="prospect-quotation-opportunity"
                  value={opportunityId}
              ...
              <div>
                <label htmlFor="prospect-quotation-valid-until">Vigencia</label>
                <input
                  id="prospect-quotation-valid-until"
                  type="date"
                  value={validUntil}
              ...
              <div>
                <label htmlFor="prospect-quotation-currency">Moneda</label>
                <input
                  id="prospect-quotation-currency"
                  value={currency}
                  maxLength={3}
              ...
              <div>
                <label htmlFor="prospect-quotation-tax">IVA %</label>
                <input
                  id="prospect-quotation-tax"
                  type="number"
                  min="0"
                  max="100"
                  step="0.01"
              ...
              <div>
                <label htmlFor="prospect-quotation-notes">Notas</label>
                <textarea
                  id="prospect-quotation-notes"
                  value={notes}
              ...
              <section>
                <div className="panel-heading">
                  <div><h4>Conceptos</h4></div>
                  <button type="button" onClick={() => setItems((current) => [...current, createEmptyItem()])}>
                    Agregar concepto
                  </button>
                </div>
                ...
                {items.map((item, index) => (
                  <div key={item.key}>
                    ...
                    <div>
                      <label>Importe</label>
                      <span>{(Number(item.quantity || 0) * Number(item.unit_price || 0)).toFixed(2)}</span>
                    </div>
                    <button
                      type="button"
              ...
              <div className="prospect-modal-notice">
                Subtotal: <strong>{subtotal.toFixed(2)}</strong> · IVA: <strong>{tax.toFixed(2)}</strong> · Total: <strong>{total.toFixed(2)} {currency.toUpperCase()}</strong>
              </div>
