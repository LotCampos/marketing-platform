              ...
              <section>
                ...
                {items.map((item, index) => (
                  <div key={item.key}>
                    ...
                    <div><label>Periodo a evaluar</label><input value={item.evaluation_period ?? ''} onChange={(event) => updateItem(item.key, 'evaluation_period', event.target.value)} placeholder="2026" /></div>
                    <div><label>Unidad</label><input value={item.unit ?? ''} onChange={(event) => updateItem(item.key, 'unit', event.target.value)} placeholder="Estación / producto / mangueras" /></div>
