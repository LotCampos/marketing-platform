...
const STATUS_COLORS = ['#f36517', '#38bdf8', '#a78bfa', '#fbbf24', '#22c55e', '#ef4444', '#94a3b8']
  ...
  return (
    <div className="chart-shell">
      <div className="chart-legend">
        {series.map((item) => <span key={item.key} className={`legend-item legend-${item.key}`}><i />{item.label}</span>)}
      </div>
      {data.length === 0 ? <p className="empty-state">Sin actividad para los filtros seleccionados.</p> : (
        <svg className="activity-chart" viewBox={`0 0 ${width} ${height}`} role="img" aria-label="Evolución de actividad comercial">
          ...
          {series.map((item) => <polyline key={item.key} points={data.map((entry, index) => point(index, entry[item.key])).join(' ')} className={`chart-line chart-line-${item.key}`} />)}
          {data.map((entry, index) => <text key={`${entry.label}-${index}`} x={padding.left + (data.length <= 1 ? innerWidth / 2 : (index / (data.length - 1)) * innerWidth)} y={height - 12} textAnchor="middle" className="chart-axis-label">{entry.label}</text>)}
        </svg>
      )}
    </div>
  )
    ...
    <div className="funnel-chart">
      {values.map((item) => (
        <div className="funnel-row" key={item.label}>
          ...
          <div className="funnel-track"><div className="funnel-bar" style={{ width: `${Math.max(8, (item.value / max) * 100)}%` }} /></div>
        </div>
      ))}
    </div>
  )
...
function StatusDistribution({ data }: { data: Array<{ label: string; value: number }> }) {
  ...
  const segments = data.map((item, index) => {
    const start = cursor
    cursor += (item.value / Math.max(1, total)) * 360
    return `${STATUS_COLORS[index % STATUS_COLORS.length]} ${start}deg ${cursor}deg`
  })

  return (
    <div className="status-chart">
      <div className="status-donut" style={{ background: data.length ? `conic-gradient(${segments.join(', ')})` : 'rgba(255,255,255,.08)' }}>
        <div className="status-donut-center"><strong>{total}</strong><span>prospectos</span></div>
      </div>
      <div className="status-list">
        {data.map((item, index) => <div className="status-list-row" key={item.label}><span className="status-dot" style={{ background: STATUS_COLORS[index % STATUS_COLORS.length] }} /><span>{item.label}</span><strong>{item.value}</strong></div>)}
        {!data.length && <p className="empty-state">Sin prospectos para los filtros seleccionados.</p>}
      </div>
    </div>
  )
}
...
export default function CommercialDashboardPage() {
  ...
  const filteredProspects = useMemo(() => prospectRows.filter((item) =>
    (!cutoff || new Date(item.created_at) >= cutoff) &&
  ...
  const filteredOpportunities = useMemo(() => opportunityRows.filter((item) => {
    ...
    return (!cutoff || new Date(item.created_at) >= cutoff) && responsibleMatch && serviceMatch && statusMatch && relationshipMatch
  ...
  const filteredQuotations = useMemo(() => quotationRows.filter((item) => {
    ...
    const statusMatch = status === 'all' || prospect?.status === status
    return (!cutoff || new Date(item.created_at) >= cutoff) && filteredOpportunityIdSet.has(item.opportunity_id) && serviceMatch && responsibleMatch && statusMatch
  ...
  const filteredAgreements = useMemo(() => agreementRows.filter((item) => {
    const opportunity = opportunityById.get(item.opportunity_id)
    const prospect = opportunity?.prospect ? prospectById.get(opportunity.prospect) : null
    const quotation = quotationRows.find((row) => row.id === item.quotation_id)
    const responsibleMatch = responsible === 'all' || opportunity?.assigned_to === responsible || prospect?.assigned_to === responsible
    const serviceMatch = service === 'all' || prospect?.service_catalog_id === service || quotation?.items.some((line) => line.service_catalog_id === service)
    const statusMatch = status === 'all' || prospect?.status === status
    return (!cutoff || new Date(item.created_at) >= cutoff) && filteredOpportunityIdSet.has(item.opportunity_id) && responsibleMatch && serviceMatch && statusMatch
  }), [agreementRows, opportunityById, prospectById, quotationRows, filteredOpportunityIdSet, responsible, service, status, cutoff])

  const filteredRequests = useMemo(() => requestRows.filter((item) => !cutoff || new Date(item.created_at) >= cutoff), [requestRows, cutoff])
  ...
  const activity = useMemo(() => {
    ...
    return [...keys].sort().slice(-6).map((key) => ({
  ...
  const responsiblePerformance = useMemo(() => {
    ...
    return [...counts.entries()].map(([key, value]) => ({ name: key === 'unassigned' ? 'Sin asignar' : userById.get(key)?.full_name ?? shortId(key), ...value })).sort((a, b) => b.value - a.value).slice(0, 6)
        ...
        <main className="page-container dashboard-page">
          <header className="page-header">
            <div>
              ...
              <p className="page-description">Vista ejecutiva del ciclo comercial con indicadores, pipeline económico, actividad y alertas operativas.</p>
            </div>
          </header>

          <section className="dashboard-filters" aria-label="Filtros comerciales">
            <label><span>Periodo</span><select value={period} onChange={(event) => setPeriod(event.target.value)}>{PERIODS.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</select></label>
            <label><span>Responsable</span><select value={responsible} onChange={(event) => setResponsible(event.target.value)}><option value="all">Todos</option>{users.filter((item) => item.is_active).map((item) => <option key={item.id} value={item.id}>{item.full_name}</option>)}</select></label>
            <label><span>Servicio</span><select value={service} onChange={(event) => setService(event.target.value)}><option value="all">Todos</option>{services.filter((item) => item.is_active).map((item) => <option key={item.id} value={item.id}>{item.service_name}</option>)}</select></label>
            <label><span>Estado</span><select value={status} onChange={(event) => setStatus(event.target.value)}><option value="all">Todos</option>{Object.entries(STATUS_LABELS).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label>
          </section>
          ...
          <section className="kpi-grid">
            ...
            <CommercialKpiCard label="Oportunidades" value={filteredOpportunities.length} description="Oportunidades en vista" />
          ...
          <section className="dashboard-grid dashboard-grid-main">
            <article className="dashboard-panel chart-panel"><header className="panel-heading"><div><span className="eyebrow">TENDENCIA</span><h2>Actividad comercial</h2></div><strong>{loading ? 'Cargando…' : `${filteredProspects.length + filteredOpportunities.length + filteredQuotations.length + filteredAgreements.length} eventos`}</strong></header><ActivityChart data={activity} /></article>
            <article className="dashboard-panel chart-panel"><header className="panel-heading"><div><span className="eyebrow">CONVERSIÓN</span><h2>Embudo comercial</h2></div></header><FunnelChart values={[{ label: 'Prospectos', value: filteredProspects.length }, { label: 'Oportunidades', value: filteredOpportunities.length }, { label: 'Cotizaciones', value: filteredQuotations.length }, { label: 'Contratos', value: filteredAgreements.length }]} /></article>
          </section>

          <section className="dashboard-grid dashboard-grid-secondary">
            <article className="dashboard-panel chart-panel"><header className="panel-heading"><div><span className="eyebrow">DISTRIBUCIÓN</span><h2>Estado de prospectos</h2></div></header><StatusDistribution data={statusDistribution} /></article>
            <article className="dashboard-panel chart-panel"><header className="panel-heading"><div><span className="eyebrow">RESPONSABLES</span><h2>Rendimiento comercial</h2></div></header><div className="performance-list">{responsiblePerformance.length === 0 && <p className="empty-state">Sin datos para los filtros seleccionados.</p>}{responsiblePerformance.map((item) => { const max = Math.max(1, ...responsiblePerformance.map((row) => row.value)); return <div className="performance-row" key={item.name}><div className="performance-head"><span>{item.name}</span><strong>{money(item.value)}</strong></div><div className="performance-track"><div style={{ width: `${Math.max(4, (item.value / max) * 100)}%` }} /></div><small>{item.opportunities} oportunidades · {item.quotations} cotizaciones</small></div> })}</div></article>
          </section>

          <section className="dashboard-panel alert-panel"><header className="panel-heading"><div><span className="eyebrow">ATENCIÓN OPERATIVA</span><h2>Alertas del pipeline</h2></div></header><div className="alert-grid"><div className="alert-card"><strong>{alerts.withoutQuotation}</strong><span>Oportunidades sin cotización</span></div><div className="alert-card"><strong>{alerts.expiring}</strong><span>Cotizaciones por vencer en 7 días</span></div><div className="alert-card"><strong>{alerts.stale}</strong><span>Prospectos sin cierre por más de 14 días</span></div></div></section>

          <section className="dashboard-panel"><header className="panel-heading"><div><span className="eyebrow">PIPELINE</span><h2>Oportunidades comerciales</h2></div><strong>{filteredOpportunities.slice(0, 8).length} visibles</strong></header><CommercialTable headers={['Oportunidad', 'Título', 'Responsable', 'Valor estimado', 'Prospecto']}>{filteredOpportunities.slice(0, 8).map((item) => { const prospect = item.prospect ? prospectById.get(item.prospect) : null; return <tr key={item.id}><td className="table-primary">{item.opportunity_number}</td><td>{item.title}</td><td>{item.assigned_to ? userById.get(item.assigned_to)?.full_name ?? shortId(item.assigned_to) : 'Sin asignar'}</td><td>{money(Number(item.estimated_value ?? 0))}</td><td>{prospect?.business_name ?? '—'}</td></tr> })}</CommercialTable></section>

          <section className="dashboard-panel"><header className="panel-heading"><div><span className="eyebrow">ACTIVIDAD</span><h2>Solicitudes recientes</h2></div><strong>{filteredRequests.slice(0, 8).length} visibles</strong></header><CommercialTable headers={['Solicitud', 'Solicitante', 'Fecha', 'Descripción']}>{filteredRequests.slice(0, 8).map((item) => <tr key={item.id}><td className="table-primary">{item.request_number}</td><td>{item.requested_by_name ?? '—'}</td><td>{item.created_at ? new Date(item.created_at).toLocaleDateString('es-MX') : '—'}</td><td><span className="commercial-table-description">{item.description ?? 'Sin descripción'}</span></td></tr>)}</CommercialTable></section>
        </main>
      </div>
    </div>
  )
