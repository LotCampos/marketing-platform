...
class CommercialBaseViewSet(viewsets.ModelViewSet):
    ...
    def get_permissions(self):
        permission_class = self.permission_by_action.get(getattr(self, "action", None))
...
class ServiceRequestViewSet(CommercialBaseViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        data = CreateServiceRequestDTO(
            ...
            service_catalog_id=serializer.validated_data["service_catalog_id"],
            request_number=serializer.validated_data["request_number"],
        ...
        response_serializer = self.get_serializer(ServiceRequest.objects.get(id=service_request.id))
...
class ProspectViewSet(CommercialBaseViewSet):
    ...
    def change_status(self, request, pk=None):
        ...
        try:
            ...
            return Response(
                {"code": "OPTIMISTIC_LOCK_CONFLICT", "detail": "Prospect was modified concurrently."},
                status=status.HTTP_409_CONFLICT,
            )
        except ApplicationValidationError as exc:
            raise DRFValidationError({"detail": exc.message_dict}) from exc
        return Response(self.get_serializer(prospect).data, status=status.HTTP_200_OK)
    ...
    def assign(self, request, pk=None):
        ...
        expected_version = request.data.get("expected_version")
        if not assigned_to:
            raise DRFValidationError({"assigned_to": "assigned_to is required."})
        if expected_version is None:
            raise DRFValidationError({"expected_version": "expected_version is required."})
        try:
...
class CapacityAssessmentViewSet(CommercialBaseViewSet):
    queryset = CapacityAssessment.objects.all().order_by("-created_at")
    serializer_class = CapacityAssessmentSerializer


class OpportunityViewSet(CommercialBaseViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        try:
            opportunity = OpportunityService().create(data)
        except ApplicationValidationError as exc:
            raise DRFValidationError({"detail": exc.message_dict}) from exc
        response_serializer = self.get_serializer(opportunity)
        headers = self.get_success_headers(response_serializer.data)
        return Response(response_serializer.data, status=status.HTTP_201_CREATED, headers=headers)


class QuotationViewSet(CommercialBaseViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        try:
            ...
            raise DRFValidationError({"detail": exc.message_dict}) from exc
        response_serializer = self.get_serializer(quotation)
        headers = self.get_success_headers(response_serializer.data)
        return Response(response_serializer.data, status=status.HTTP_201_CREATED, headers=headers)
...
class QuotationItemViewSet(CommercialBaseViewSet):
    queryset = QuotationItem.objects.all().order_by("-created_at")
...
class AgreementTermViewSet(CommercialBaseViewSet):
    queryset = AgreementTerm.objects.all().order_by("-created_at")
